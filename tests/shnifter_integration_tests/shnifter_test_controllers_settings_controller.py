...
import pytest
from shnifter_interface.controllers.settings_controller import SettingsController
...
@pytest.fixture
def mock_session():
    with patch("shnifter_interface.controllers.settings_controller.session") as mock:
...
def test_call_shnifterject_display(mock_session):
    controller = SettingsController()
    args = ["--value", "5"]
    controller.call_shnifterject_display(args)
    mock_session.settings.set_item.assert_called_once_with(
        "N_TO_DISPLAY_ShnifterJECT_REGISTRY", 5
    )


def test_call_shnifterject_display_no_args(mock_session):
    mock_session.settings.N_TO_DISPLAY_ShnifterJECT_REGISTRY = 10
    controller = SettingsController()
    controller.call_shnifterject_display([])
