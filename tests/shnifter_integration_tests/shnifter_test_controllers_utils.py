...
import pytest
from shnifter_interface.controllers.utils import (
    check_non_negative,
    check_positive,
...
def mock_session():
    ...
    with patch("shnifter_interface.controllers.utils.session") as mock_session:
        mock_session.console.print = MagicMock()
        mock_session.is_local = MagicMock(return_value=True)
        mock_session.settings.VERSION = "1.0"
        mock_session.user.profile.core_session.username = "testuser"
...
def test_welcome_message(mock_session):
    ...
    mock_session.console.print.assert_called_with(
        "\nWelcome to Shnifter Engine Interface v1.0"
    )
