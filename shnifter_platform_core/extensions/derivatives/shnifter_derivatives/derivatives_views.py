# Generated: 2025-07-04T09:50:40.191147
    ...
    def derivatives_futures_curve(  # noqa: PLR0912
        **kwargs,
        ...
        data : Optional[Union[List[Data], DataFrame]]
            ...
            If not supplied, the original Shnifterject.results will be used.
        ...
        -------
        Tuple[ShnifterFigure, Dict[str, Any]]
            Tuple with the ShnifterFigure object, and the JSON-serialized content.
        ...
        data = shnifter.derivatives.futures.curve(symbol="vx", provider="cboe", date=["2020-03-31", "2024-06-28"], chart=True)
        data.show()
        ```
        ...
        if data:
            ...
            df = DataFrame(
                [
                    ...
                    for d in kwargs["shnifterject_item"]
                ]
                if isinstance(kwargs.get("shnifterject_item"), list)
                else kwargs["shnifterject_item"].model_dump(exclude_none=True, exclude_unset=True)  # type: ignore
            )

        if df.empty:
            raise ShnifterError("Error: No data to plot.")

        if "expiration" not in df.columns:
            raise ShnifterError("Expiration field not found in the data.")
        ...
        figure = ShnifterFigure().create_subplots(shared_xaxes=True)
