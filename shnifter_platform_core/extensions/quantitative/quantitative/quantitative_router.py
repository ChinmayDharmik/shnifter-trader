# Generated: 2025-07-04T09:50:40.098425
...
from shnifter_quantitative.models import (
    ADFTestModel,
    CAPMModel,
    KPSSTestModel,
    NormalityModel,
...
from shnifter_quantitative.performance.performance_router import (
    router as performance_router,
)
from shnifter_quantitative.rolling.rolling_router import router as rolling_router
from shnifter_quantitative.stats.stats_router import router as stats_router
    ...
    examples=[
        PythonEx(
            ...
            code=[
                ...
                "shnifter.quantitative.normality(data=stock_data, target='close')",
            ],
        ),
        APIEx(parameters={"target": "close", "data": APIEx.mock_data("timeseries", 8)}),
    ],
)
def normality(data: list[Data], target: str) -> Shnifterject[NormalityModel]:
    ...
    return Shnifterject(results=norm_summary)
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = shnifter.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501
                "shnifter.quantitative.capm(data=stock_data, target='close')",
            ],
        ),
        APIEx(
...
def capm(data: list[Data], target: str) -> Shnifterject[CAPMModel]:
    ...
    from shnifter_quantitative.helpers import get_fama_raw  # noqa
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = shnifter.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501
                "shnifter.quantitative.unitroot_test(data=stock_data, target='close')",
            ],
        ),
...
def unitroot_test(
    ...
    return Shnifterject(results=unitroot_summary)
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = shnifter.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501
                "shnifter.quantitative.summary(data=stock_data, target='close')",
            ],
        ),
        APIEx(parameters={"target": "close", "data": APIEx.mock_data("timeseries", 5)}),
    ],
)
def summary(data: list[Data], target: str) -> Shnifterject[SummaryModel]:
