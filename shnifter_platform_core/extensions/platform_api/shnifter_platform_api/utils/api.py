# Generated: 2025-07-04T09:50:40.111784
...
Serve the Shnifter Engine API.


Launcher specific arguments:

    --app                           Absolute path to the Python file with the target FastAPI instance. Default is the installed Shnifter Engine API.
    ...
    --login                         Login to the Shnifter Engine.
    ...
    --apps-json                     Absolute/relative path to use as the apps.json file. Default is ~/ShnifterUserData/workspace_apps.json.
...
>>> from shnifter_engine_api.main import app
>>>
>>> @app.get()
...
All other arguments will be passed to uvicorn. Here are the most common ones:
    ...
    --ssl-version INTEGER           SSL version to use.
                                      ...
                                      [default: 17]
    --ssl-cert-reqs INTEGER         Whether interfaceent certificate is required.
                                      (see stdlib ssl module's)
                                      [default: 0]
...
def get_user_settings(
    ...
    if pat:
        from shnifter_core.app.service.core_service import CoreService

        core_credentials: dict = {}
        core_preferences: dict = {}
        core_defaults: dict = {}
        try:
            Core = CoreService()
            _ = Core.connect(pat=pat)
            core_settings = Core.pull()
            core_credentials = json.loads(
                core_settings.credentials.model_dump_json()  # pylint: disable=no-member
            )
            core_preferences = json.loads(
                core_settings.preferences.model_dump_json()  # pylint: disable=no-member
            )
            core_defaults = json.loads(
                core_settings.defaults.model_dump_json()  # pylint: disable=no-member
            )
        except Exception as e:  # pylint: disable=broad-exception-caught
            print(  # noqa: T201
                f"\n\nError connecting with Core:\n{e}\n\nUsing the local settings.\n"
            )
        ...
        if core_credentials:
            for k, v in core_credentials.items():
                if v:
                    new_settings["credentials"][k] = v.strip('"').strip("'")

        if core_preferences:
            for k, v in core_preferences.items():
                if v:
        ...
        if core_defaults:
            for k, v in core_defaults.items():
                if k == "commands":
                    for key, value in core_defaults["commands"].items():
                        if value:
...
def get_widgets_json(
    ...
    """Generate and serve the widgets.json for the Shnifter Engine API."""
    if editable is True:
...
def import_app(app_path: str, name: str = "app", factory: bool = False):
    ...
    from shnifter_core.api.rest_api import system
    ...
    # See: https://gitcore.com/encode/uvicorn/blob/master/uvicorn/config.py
    try:
        app = app_or_factory()
        if not factory:
            print(  # noqa: T201
