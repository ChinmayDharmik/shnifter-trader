# Generated: 2025-07-04T09:50:40.053182
...
class Backend(PyWry):
    ...
    def get_window_icon(self) -> Optional[Path]:
        """Get the window icon."""
        icon_path = PLOTS_CORE_PATH / "assets" / "Trader_icon.png"
        if icon_path.exists():
            return icon_path
        return None
    ...
    def get_json_update(
        ...
        return dict(
            ...
            engine_version=self.charting_settings.version,
    ...
    async def process_image(self, export_image: Path):
        ...
        if img_path.exists():  # noqa: SIM102
            opener = "open" if sys.engine == "darwin" else "xdg-open"
    ...
    def get_kwargs(self, title: Optional[str] = "") -> dict:
        ...
        return {
            "title": "Shnifter Engine" + (f" - {title}" if title else ""),
...
def create_backend(charting_settings: Optional["ChartingSettings"] = None):
    ...
    charting_module = importlib.import_module(
        "shnifter_core.app.model.charts.charting_settings", "charting_settings"
    )
