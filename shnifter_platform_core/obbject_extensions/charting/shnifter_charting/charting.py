# Generated: 2025-07-04T09:50:40.040863
...
from shnifter_core.app.model.charts.chart import Chart
from shnifter_core.app.model.shnifterject import Shnifterject
from shnifter_core.provider.abstract.data import Data

from shnifter_charting.charts.helpers import (
...
if TYPE_CHECKING:
    ...
    from shnifter_charting.core.shnifter_figure import ShnifterFigure  # noqa
    from shnifter_charting.query_params import ChartParams  # noqa
    from shnifter_charting.core.backend import Backend  # noqa
...
class Charting:
    ...
    -------
    show
        Display chart and save it to the Shnifterject.
    to_chart
        Redraw the chart and save it to the Shnifterject, with an optional entry point for Data.
    functions
        Return a list of Engine commands with charting functions.
    get_params
        Return the charting parameters for the function the Shnifterject was created from.
    indicators
    ...
    _extension_views: ClassVar[List[Type]] = [
        entry_point.load()
        for entry_point in entry_points(group="shnifter_charting_extension")
    ]
    _format = "plotly"  # the charts computed by this extension will be in plotly format

    def __init__(self, shnifterject):
        """Initialize Charting extension."""
        # pylint: disable=import-outside-toplevel
        import importlib  # noqa

        charting_settings_module = importlib.import_module(
            "shnifter_core.app.model.charts.charting_settings", "ChartingSettings"
        )
        ChartingSettings = charting_settings_module.ChartingSettings

        self._shnifterject: Shnifterject = shnifterject
        self._charting_settings = ChartingSettings(
            user_settings=self._shnifterject._user_settings,  # type: ignore
            system_settings=self._shnifterject._system_settings,  # type: ignore
        )
    ...
    def indicators(cls):
        ...
        """
        # pylint: disable=import-outside-toplevel
        from shnifter_charting.query_params import IndicatorsParams
    ...
    def _handle_backend(self) -> "Backend":
        ...
        # pylint: disable=import-outside-toplevel
        from shnifter_charting.core.backend import create_backend, get_backend
    ...
    def _get_chart_function(self, route: str) -> Callable:
        ...
        if route is None:
            raise ValueError("Shnifterject was initialized with no function route.")
    ...
    def get_params(self) -> Union["ChartParams", None]:
        """Return the ChartQueryParams class for the function the Shnifterject was created from.
        ...
        """
        # pylint: disable=import-outside-toplevel
        from shnifter_charting.query_params import ChartParams

        if self._shnifterject._route is None:  # pylint: disable=protected-access
            raise ValueError("Shnifterject was initialized with no function route.")
        charting_function = (
            self._shnifterject._route  # pylint: disable=protected-access
        ).replace("/", "_")[1:]
        ...
        return help(  # type: ignore
            self._get_chart_function(  # pylint: disable=protected-access
                self._shnifterject.extra[  # pylint: disable=protected-access
                    "metadata"
                ].route
            )
        )
    ...
    def _prepare_data_as_df(
        ...
        # pylint: disable=import-outside-toplevel
        from shnifter_core.app.utils import basemodel_to_df, convert_to_basemodel
        ...
        data_as_df: DataFrame = (
            ...
            if has_data
            else self._shnifterject.to_dataframe(index=index)
        )
    ...
    def create_line_chart(
        ...
        render: bool = True,
        **kwargs,
    ) -> Union["ShnifterFigure", "Figure", None]:
        """Create a line chart from external data and render a chart or return the ShnifterFigure.
        ...
        **kwargs: Dict[str, Any]
        ...
        """
        # pylint: disable=import-outside-toplevel
        from shnifter_charting.charts.generic_charts import line_chart
    ...
    def create_bar_chart(
        ...
        **kwargs,
        ...
        -------
        ShnifterFigure
            The ShnifterFigure object.
        """
        # pylint: disable=import-outside-toplevel
        from shnifter_charting.charts.generic_charts import bar_chart
    ...
    def show(self, render: bool = True, **kwargs):
        ...
        try:
            ...
            kwargs["shnifterject_item"] = self._shnifterject.results
            kwargs["charting_settings"] = self._charting_settings
            kwargs["standard_params"] = (
                self._shnifterject._standard_params  # pylint: disable=protected-access
            )
            kwargs["extra_params"] = (
                self._shnifterject._extra_params  # pylint: disable=protected-access
            )
            kwargs["provider"] = self._shnifterject.provider
            kwargs["extra"] = self._shnifterject.extra
            fig, content = charting_function(**kwargs)
            self._shnifterject.chart = Chart(fig=fig, content=content, format=self._format)
            if render:
                fig.show(**kwargs)

        except (RuntimeError, ShnifterError) as e:
            raise e from e

        except Exception:  # pylint: disable=W0718
            try:
                fig = self.create_line_chart(data=self._shnifterject.results, render=False, **kwargs)  # type: ignore
                fig = self._set_chart_style(fig)
                content = fig.show(external=True, **kwargs).to_plotly_json()  # type: ignore
                self._shnifterject.chart = Chart(
    ...
    def to_chart(
        ...
        **kwargs,
        ...
        """Create an ShnifterFigure with user customizations (if any) and save it to the Shnifterject.

        This function is used to populate, or re-populate, the Shnifterject with a chart using the data within
        the Shnifterject or external data supplied via the `data` parameter.
        This function modifies the original Shnifterject by overwriting the existing chart.
        ...
        >>> res = shnifter.equity.price.historical("AAPL")
        >>> indicators = dict(
        ...
        kwargs["provider"] = self._shnifterject.provider  # pylint: disable=protected-access
        kwargs["extra"] = self._shnifterject.extra  # pylint: disable=protected-access
        try:
            if has_data:
    ...
    def toggle_chart_style(self):
        ...
        if not hasattr(self._shnifterject.chart, "fig"):
            raise ValueError(
        ...
        figure = self._shnifterject.chart.fig  # type: ignore[union-attr]
        updated_figure = self._set_chart_style(figure)
        self._shnifterject.chart.fig = updated_figure  # type: ignore[union-attr]
        self._shnifterject.chart.content = updated_figure.show(  # type: ignore[union-attr]
            external=True
        ).to_plotly_json()
    ...
    def table(
        ...
        data : Optional[Union[DataFrame, Series]], optional
            ...
            If no data is provided the Shnifterject results will be used.
        title : str, optional
        ...
        if self._backend.isatty:
            try:
                self._backend.send_table(
                    df_table=data_as_df,
                    title=title
                    or self._shnifterject._route,  # pylint: disable=protected-access
