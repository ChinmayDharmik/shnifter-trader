# Generated: 2025-07-04T09:50:40.498753
...
try:
    from shnifter_charting import Charting
...
def validate_output(c_out: Shnifterject) -> Shnifterject:
    """
    Validate Shnifterject object.

    Checks against the Shnifterject schema and removes fields that contain the
    `exclude_from_api` extra `pydantic.Field` kwarg.
    Note that the modification to the `Shnifterject` object is done in-place.

    Parameters
    ----------
    c_out : Shnifterject
        Shnifterject object to validate.

    Returns
    -------
    Dict
        Serialized Shnifterject.
    """
    ...
    # Let a non-Shnifterject object pass through without validation
    if not isinstance(c_out, Shnifterject):
        return c_out
...
def build_api_wrapper(
    ...
    @wraps(wrapped=func)
    async def wrapper(*args: Tuple[Any], **kwargs: Dict[str, Any]) -> Shnifterject:
        ...
        if isinstance(output, Shnifterject) and not no_validate:
