# Generated: 2025-07-04T09:50:40.434664
...
class OptionsChainsProperties(Data):
    ...
    def dataframe(self) -> "DataFrame":
        ...
        if "underlying_price" not in chains_data.columns and not self.last_price:
            raise ShnifterError(
                ...
                + "\n\n Note: This error does not impact the standard Shnifterject `to_df()` method."
            )
        ...
        if chains_data.empty:
            raise ShnifterError("Error: No validated data was found.")
    ...
    def total_dex(self) -> Dict:
        ...
        if not self.has_greeks:
            raise ShnifterError("Greeks are not available.")
    ...
    def filter_data(
        ...
        if stat is not None:
            if stat not in stats:
                raise ShnifterError(f"Error: stat must be one of {stats}")
        ...
        if column is not None:
            if column not in df.columns:
                raise ShnifterError(f"Error: column '{column}' not found in data")
    ...
    def _get_stat(
        ...
        if metric in ["DEX", "GEX"]:
            if not self.has_greeks:
                raise ShnifterError("Greeks were not found within the data.")
    ...
    def _get_nearest_strike(
        ...
        if option_type not in ["call", "put"]:
            raise ShnifterError("Error: option_type must be either 'call' or 'put'")
    ...
    def strategies(  # noqa: PLR0912
        ...
        if strategies.empty:
            raise ShnifterError("No strategies found for the given parameters.")
    ...
    def skew(
        ...
        if self.has_iv is False:
            raise ShnifterError("Error: 'implied_volatility' field not found.")
