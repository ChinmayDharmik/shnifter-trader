# Generated: 2025-07-04T09:50:40.459947
...
if TYPE_CHECKING:
    from shnifter_core.app.static.app_factory import BaseApp
...
class Account:  # noqa: D205, D400
    ...
    SESSION_FILE = ".core_session.json"

    def __init__(self, base_app: "BaseApp"):
        """Initialize account service."""
        self._base_app = base_app
        self._shnifter_directory = (
            base_app._command_runner.system_settings.shnifter_directory
        )
        self._core_service: Optional[CoreService] = None
    ...
    def _create_core_service(
        self,
        email: Optional[str] = None,
        password: Optional[str] = None,
        pat: Optional[str] = None,
    ) -> CoreService:
        """Create core service to handle connection."""
        if email is None and password is None and pat is None:
            session_file = Path(self._shnifter_directory, self.SESSION_FILE)
            if not session_file.exists():
                raise ShnifterError("Session not found.")

            with open(session_file) as f:
                session_dict = json.load(f)

            core_session = CoreSession(**session_dict)
            hs = CoreService(core_session)
        else:
            hs = CoreService()
    ...
    def login(
        ...
        """
        self._core_service = self._create_core_service(email, password, pat)
        incoming = self._core_service.pull()
        ...
        if remember_me:
            Path(self._shnifter_directory).mkdir(parents=False, exist_ok=True)
            session_file = Path(self._shnifter_directory, self.SESSION_FILE)
            with open(session_file, "w") as f:
                if not self._core_service.session:
                    raise ShnifterError("Not connected to core.")

                json.dump(
                    self._core_service.session.model_dump(mode="json"), f, indent=4
                )
    ...
    def save(self, return_settings: bool = False) -> Optional[UserSettings]:
        ...
        if not self._core_service:
            ...
            self._core_service.push(self._base_app._command_runner.user_settings)
