# Generated: 2025-07-04T09:50:40.467860
...
from shnifter_core.app.extension_loader import ExtensionLoader, ShnifterGroups
from shnifter_core.app.model.example import Example
...
from shnifter_core.app.static.utils.console import Console
from shnifter_core.app.static.utils.linters import Linters
...
if TYPE_CHECKING:
    ...
    from shnifter_core.provider.abstract.data import Data  # noqa

try:
    from shnifter_charting import Charting  # type: ignore
...
class PackageBuilder:
    """Build the extension package for the Engine."""
    ...
    def build(
        ...
        """Build the extensions for the Engine."""
    ...
    def _save_reference_file(self, ext_map: Optional[Dict[str, List[str]]] = None):
        ...
        code = dumps(
            obj={
                "shnifter": VERSION.replace("dev", ""),
                "info": {
                    "title": "Shnifter Engine (Python)",
    ...
    def _diff(ext_map: Dict[str, List[str]]) -> Tuple[Set[str], Set[str]]:
        ...
        ext_map: Dict[str, List[str]]
            ...
            Example:
                {
                    "shnifter_core_extension": [
                        "commodity@1.0.1",
                        ...
                    ],
                    "shnifter_provider_extension": [
                        "benzinga@1.1.3",
                        ...
                    ],
                    "shnifter_shnifterject_extension": [
                        "shnifter_charting@1.0.0",
                        ...
                    ]
                }
...
class ModuleBuilder:
    """Build the module for the Engine."""
...
class ImportDefinition:
    """Build the import definition for the Engine."""
    ...
    def get_function_hint_type_list(cls, route) -> List[Type]:
        ...
        if return_type:
            hint_type = (
                ...
                and "Shnifterject" in return_type.__class__.__name__
                else return_type
            )
    ...
    def build(cls, path: str) -> str:
        ...
        code = "from shnifter_core.app.static.container import Container"
        code += "\nfrom shnifter_core.app.model.shnifterject import Shnifterject"
...
class ClassDefinition:
    """Build the class definition for the Engine."""
    ...
    def build(path: str, ext_map: Optional[Dict[str, List[str]]] = None) -> str:
        ...
        if not path:
            if ext_map:
                ...
                doc += "Extensions:\n"
                doc += "\n".join(
                    [f"    - {ext}" for ext in ext_map.get("shnifter_core_extension", [])]
                )
                doc += "\n\n"
                doc += "\n".join(
                    [
                        f"    - {ext}"
                        for ext in ext_map.get("shnifter_provider_extension", [])
                    ]
                )
            doc += '    """\n'
            doc += "    # fmt: on\n"
...
class MethodDefinition:
    """Build the method definition for the Engine."""
        ...
        for name, param in parameter_map.items():
            ...
            if isinstance(param.annotation, _AnnotatedAlias):
                ...
                if query_obj:
                    ...
                    # Create a new annotation with ShnifterField containing the description
        ...
        for param, value in od.items():
            ...
            new_value = value.replace(
                annotation=Annotated[
                    ...
                    ShnifterField(description=description),
                ],
            )
    ...
    def build_func_params(formatted_params: OrderedDict[str, Parameter]) -> str:
        ...
        func_params = func_params.replace(
            "shnifter_core.provider.abstract.data.Data", "Data"
        )
    ...
    def build_func_returns(return_type: type) -> str:
        ...
        elif isclass(return_type) and issubclass(return_type, Shnifterject):
            func_returns = "Shnifterject"
        else:
...
class DocstringGenerator:
    ...
    def get_field_type(
        ...
        try:
            ...
            if "shnifter_" in str(_type):
                _type = (
                    str(_type).split(".", maxsplit=1)[0].split("shnifter_")[0]
    ...
    @staticmethod
    def get_Shnifterject_description(
        results_type: str,
        ...
        shnifterject_description = (
            f"{create_indent(indent)}Shnifterject\n"
        ...
        shnifterject_description = shnifterject_description.replace("NoneType", "None")

        return shnifterject_description
    ...
    def build_examples(
        ...
        if examples:
            ...
            doc = f"\n{indent}Examples\n"
            doc += f"{indent}--------\n"
            doc += f"{indent}{prompt}from shnifter import shnifter\n"
    ...
    def generate_model_docstring(  # noqa: PLR0912, PLR0917
        ...
        if "parameters" in sections:
            ...
            for param_name, param in kwarg_params.items():
                ...
                # If empty description, check for ShnifterField annotations in parameter's annotation
                if not description and hasattr(param, "annotation"):
                    ...
                    if (
                        ...
                        for meta in metadata:
                            # Look for ShnifterField with description
        ...
        if "returns" in sections:
            ...
            docstring += cls.get_Shnifterject_description(results_type, _providers)
        ...
        if model_name:
            ...
            if params and return_schema:
                ...
                results_type = (
                    ...
                    if isclass(annotation) and issubclass(annotation, Shnifterject)  # type: ignore[arg-type]
                    else model_name
                )
            ...
            if (
                ...
                for param_name, param in formatted_params.items():
                    ...
                    if isinstance(annotation, _AnnotatedAlias):
                        # Extract from ShnifterField annotations
...
class PathHandler:
    """Handle the paths for the Engine."""
...
class ReferenceGenerator:
    """Generate the reference for the Engine."""
    ...
    def _get_provider_field_params(
        cls, model: str, params_type: str, provider: str = "shnifter"
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _get_shnifterject_returns_fields(
        model: str,
        providers: str,
    ) -> List[Dict[str, str]]:
        """Get the fields of the Shnifterject returns object for the given standard_model.
    ...
    def _get_post_method_parameters_info(
        ...
        if matches:
            ...
            for match in matches:
                ...
                param_type = (
                    str(param_type)
                    .replace("shnifter_core.provider.abstract.data.Data", "Data")
                    .replace("List", "list")
                    .replace("Dict", "dict")
                    .replace("NoneType", "None")
                )
    ...
    def _clean_string_values(value: Any) -> Any:
        ...
        if isinstance(value, str):
            ...
            value = re.sub(
                r"List\[shnifter_core\.provider\.abstract\.data\.Data\]",
                "list[Data]",
                value,
            )
            value = re.sub(
                r"shnifter_core\.provider\.abstract\.data\.Data", "Data", value
            )
    ...
    def _get_post_method_returns_info(docstring: str) -> dict:
        ...
        # This pattern captures the model name inside "Shnifterject[]" and its description
        ...
        if match:
            ...
            content_inside_brackets = re.search(
                r"Shnifterject\[\s*((?:[^\[\]]|\[[^\[\]]*\])*)\s*\]", return_type
        ...
        for path, route in route_map.items():
            ...
            if route_method == {"GET"} and model_map:
                ...
                for provider in model_map:
                    if provider == "shnifter":
                        # shnifter provider is always present hence its the standard field
                        reference[path]["parameters"]["standard"] = (
                            cls._get_provider_field_params(
                                standard_model, "QueryParams"
                            )
                        )
                        # Add `provider` parameter fields to the shnifter provider
                ...
                if validate_output is False:
                    ...
                    reference[path]["returns"]["Shnifterject"] = (
                        cls._get_shnifterject_returns_fields(standard_model, providers)
                    )
            # Add data for the endpoints without a standard model (data processing endpoints)
                ...
                if validate_output is False:
                    ...
                    is_generic_shnifterject = (
                        isinstance(returns_info, dict)
                        and "Shnifterject" in returns_info
                        and any(
                            item.get("name") == "results" and "Data" in item.get("type")
                            for item in returns_info.get("Shnifterject", [])
                        )
                    )
                    ...
                    # For Pydantic models, extract the fields
                        ...
                        and not is_generic_shnifterject
                    ):
                    ...
                    # For results field in Shnifterject returns, check for actual model type

                    if isinstance(returns_info, dict) and "Shnifterject" in returns_info:
                        # For Shnifterject returns, extract model name from results field type
                        model_name = None
                        for item in returns_info["Shnifterject"]:
                        ...
                        # For direct returns that aren't Shnifterject
                        model_name = (
    ...
    def _extract_return_type(func: Callable) -> Union[str, dict]:
        ...
        # Check if the return type is an Shnifterject
        type_str = str(return_annotation)

        if "Shnifterject" in type_str or (
            hasattr(return_annotation, "__name__")
            and "Shnifterject" in return_annotation.__name__
        ):
            ...
            if result_type == "list[Data]":
                ...
                if "Returns" in docstring:
                    ...
                    patterns = [
                        r"Shnifterject\[(.*?)\]",  # Shnifterject[Model]
            ...
            # Return the standard Shnifterject structure with correct result type
            return {
                "Shnifterject": [
                    {
                        "name": "results",
