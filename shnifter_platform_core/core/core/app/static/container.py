# Generated: 2025-07-04T09:50:40.462417
...
class Container:
    ...
    def __init__(self, command_runner: "CommandRunner") -> None:
        ...
        Shnifterject._user_settings = command_runner.user_settings
        Shnifterject._system_settings = command_runner.system_settings
    ...
    def _run(self, *args, **kwargs) -> Any:
        ...
        shnifterject = self._command_runner.sync_run(*args, **kwargs)
        output_type = self._command_runner.user_settings.preferences.output_type
        if output_type == "Shnifterject":
            return shnifterject
        return getattr(shnifterject, "to_" + output_type)()
        ...
        if choice is None:
            ...
            for p in providers:
                ...
                if result is False:
                    ...
                    tries.append((p, f"not installed, please install shnifter-{p}"))

            msg = "\n  ".join([f"* '{pair[0]}' -> {pair[1]}" for pair in tries])
            raise ShnifterError(f"Provider fallback failed.\n" f"[Providers]\n  {msg}")
        return choice
