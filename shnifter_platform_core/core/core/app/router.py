# Generated: 2025-07-04T09:50:40.233642
...
from shnifter_core.app.deprecation import DeprecationSummary, ShnifterDeprecationWarning
...
from shnifter_core.app.model.example import filter_list
...
class ShnifterErrorResponse(BaseModel):
    """Shnifter Error Response."""
...
class Router:
    ...
    @overload
    def command(self, func: Optional[Callable[P, Shnifterject]]) -> Callable[P, Shnifterject]:
        pass
    ...
    def command(
        self,
        func: Optional[Callable[P, Shnifterject]] = None,
        **kwargs,
    ) -> Optional[Callable]:
        ...
        if func := SignatureInspector.complete(func, model):
            ...
            if kwargs.get("deprecated", False):
                deprecation: ShnifterDeprecationWarning = kwargs.pop("deprecation")
...
class SignatureInspector:
    ...
    @classmethod
    def complete(
        cls, func: Callable[P, Shnifterject], model: str
    ) -> Optional[Callable[P, Shnifterject]]:
        """Complete function signature."""
        if isclass(return_type := func.__annotations__["return"]) and not issubclass(
            return_type, Shnifterject
        ):
            return func
    ...
    @staticmethod
    def polish_return_schema(func: Callable[P, Shnifterject]) -> Callable[P, Shnifterject]:
        """Polish API schemas by filling `__doc__` and `__name__`."""
        return_type = func.__annotations__["return"]
        is_list = False

        if return_type == Shnifterject:
            ...
            func.__annotations__["return"].__doc__ = "Shnifterject"
            func.__annotations__["return"].__name__ = f"Shnifterject[{inner_type_name}]"

        return func

    @staticmethod
    def validate_signature(
        func: Callable[P, Shnifterject], expected: Dict[str, type]
    ) -> None:
    ...
    def inject_dependency(
        func: Callable[P, Shnifterject], arg: str, callable_: Any
    ) -> Callable[P, Shnifterject]:
    ...
    def inject_return_annotation(
        func: Callable[P, Shnifterject], annotation: Type[Shnifterject]
    ) -> Callable[P, Shnifterject]:
    ...
    def get_operation_id(func: Callable, sep: str = "_") -> str:
        """Get operation id."""
        operation_id = [
            t.replace("_router", "").replace("shnifter_", "")
        ...
        for route in api_router.routes:
            ...
            if openapi_extra:
                ...
                if model:
                    ...
                    if "shnifter" in providers:
                        providers.remove("shnifter")
