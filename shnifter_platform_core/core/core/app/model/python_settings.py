# Generated: 2025-07-04T09:50:40.450638
...
class PythonSettings(BaseModel):
    ...
    http: Optional[dict] = Field(
        ...
        + """Available settings:
            ...
            - certfile: str - Path to a interfaceent certificate file.
            - keyfile: str - Path to a interfaceent key file.
            - password: str - Password for the interfaceent key file.  # aiohttp only
        ...
        The settings are passed into the `requests.Session` object and the `aiohttp.ClientSession` object by:
            - `shnifter_core.provider.utils.helpers.make_request` - Sync
            - `shnifter_core.provider.utils.helpers.amake_request` - Async
            - `shnifter_core.provider.utils.helpers.amake_requests` - Async (multiple requests)
            - Inserted to use with YFinance & Finviz library implementations.

        Return a session object with the settings applied by:
            - `shnifter_core.provider.utils.helpers.get_requests_session`
            - `shnifter_core.provider.utils.helpers.get_async_requests_session`
        """,
    )
    uvicorn: Optional[dict] = Field(
        ...
        + """
            - Running the FastAPI as a Python module script.
              - python -m shnifter_core.api.rest_api
            - Running the `shnifter-api` command.
                - shnifter-api
