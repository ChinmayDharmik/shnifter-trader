# Generated: 2025-07-04T09:50:40.231505
...
from shnifter_core.provider.query_executor import QueryExecutor
from shnifter_core.provider.registry_map import MapType, RegistryMap
from shnifter_core.provider.utils.helpers import to_snake_case
...
class ProviderInterface(metaclass=SingletonMeta):
    ...
    @property
    def return_annotations(self) -> Dict[str, Type[Shnifterject]]:
        """Return map."""
    ...
    def _create_field(
        ...
        provider_field = (
            f"(provider: {provider_name})" if provider_name != "shnifter" else ""
        )
        description = (
    ...
    def _extract_params(
        ...
        for provider_name, model_details in providers.items():
            if provider_name == "shnifter":
                ...
                for name, field in model_details["QueryParams"]["fields"].items():
                    if name not in providers["shnifter"]["QueryParams"]["fields"]:
            ...
            if provider_name == "shnifter":
                for name, field in model_details["Data"]["fields"].items():
                    if (
                        ...
                        continue
                    incoming = cls._create_field(name, field, "shnifter")
                ...
                for name, field in model_details["Data"]["fields"].items():
                    if name not in providers["shnifter"]["Data"]["fields"]:
                        if (
                            name == "provider"
                            and field.description == "The data provider for the data."
                        ):  # noqa
    ...
    def _generate_model_providers_dc(self, map_: MapType) -> Dict[str, ProviderChoices]:
        ...
        for model_name, providers in map_.items():
            ...
            if "shnifter" in choices:
                choices.remove("shnifter")
    ...
    def _generate_return_annotations(
        ...
        for name, models in original_models.items():
            ...
            annotations[name] = create_model(
                f"Shnifterject_{name}",
                __base__=Shnifterject[full],  # type: ignore[valid-type]
                __doc__=f"Shnifterject with results of type {name}",
            )
        return annotations
