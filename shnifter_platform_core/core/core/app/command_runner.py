# Generated: 2025-07-04T09:50:40.226734
...
from shnifter_core.app.model.abstract.warning import ShnifterWarning, cast_warning
from shnifter_core.app.model.metadata import Metadata
from shnifter_core.app.model.shnifterject import Shnifterject
from shnifter_core.app.provider_interface import ExtraParams
from shnifter_core.app.static.package_builder import PathHandler
from shnifter_core.env import Env
from shnifter_core.provider.utils.helpers import maybe_coroutine, run_async
...
class StaticCommandRunner:
    ...
    async def _command(
        ...
        shnifterject = await maybe_coroutine(func, **kwargs)
        if isinstance(shnifterject, Shnifterject):
            shnifterject.provider = getattr(
                kwargs.get("provider_choices"),
                "provider",
                getattr(shnifterject, "provider", None),
            )
        return shnifterject
    ...
    def _chart(
        cls,
        shnifterject: Shnifterject,
        **kwargs,
    ) -> None:
        """Create a chart from the command output."""
        try:
            if "charting" not in shnifterject.accessors:
                raise ShnifterError(
                    "Charting is not installed. Please install `shnifter-charting`."
                )
            # Here we will pop the chart_params kwargs and flatten them into the kwargs.
            chart_params = {}
            extra_params = getattr(shnifterject, "_extra_params", {})
            ...
            shnifterject.charting.show(render=False, **kwargs)  # type: ignore[attr-defined]
        except Exception as e:  # pylint: disable=broad-exception-caught
            if Env().DEBUG_MODE:
                raise ShnifterError(e) from e
            warn(str(e), ShnifterWarning)
    ...
    async def _execute_func(  # pylint: disable=too-many-positional-arguments
        ...
        try:
            with catch_warnings(record=True) as warning_list:
                ...
                shnifterject = await cls._command(func, kwargs)
                # The output might be from a router command with 'no_validate=True'
                # It might be of a different type than Shnifterject.
                # In this case, we avoid accessing those attributes.
                if isinstance(shnifterject, Shnifterject):
                    # This section prepares the shnifterject to pass to the charting service.
                    shnifterject._route = route  # pylint: disable=protected-access
                    ...
                    shnifterject._standard_params = (  # pylint: disable=protected-access
                        std_params
                    )
                    shnifterject._extra_params = (  # pylint: disable=protected-access
                        extra_params
                    )
                    if chart and shnifterject.results:
                        cls._chart(shnifterject, **kwargs)

                raised_warnings = warning_list if warning_list else []
        finally:
            if raised_warnings:
                if isinstance(shnifterject, Shnifterject):
                    shnifterject.warnings = []
                for w in raised_warnings:
                    if isinstance(shnifterject, Shnifterject):
                        shnifterject.warnings.append(cast_warning(w))
            ...
            if system_settings.logging_suppress is False:
                # pylint: disable=import-outside-toplevel
                from shnifter_core.app.logs.logging_service import LoggingService
    ...
    async def run(
        ...
        **kwargs,
        ...
        """Run a command and return the Shnifterject as output."""
        ...
        if func := command_map.get_command(route=route):
            shnifterject = await cls._execute_func(
                route=route,
        ...
        if execution_context.user_settings.preferences.metadata and isinstance(
            ...
            try:
                shnifterject.extra["metadata"] = Metadata(
                    arguments=kwargs,
                    duration=duration,
                    route=route,
                    timestamp=timestamp,
...
class CommandRunner:
    ...
    def __init__(
        ...
        from shnifter_core.app.service.user_service import UserService
