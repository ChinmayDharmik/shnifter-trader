# Generated: 2025-07-04T09:50:39.504723
...
from shnifter_core.provider.standard_models.index_constituents import (
...
class TmxIndexConstituentsFetcher(
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        from shnifter_tmx.utils.helpers import get_data_from_url, get_indices_backend
    ...
    def transform_data(
        ...
        if query.symbol not in data.get("indices"):  # type: ignore
            raise ShnifterError(f"Index {query.symbol} was not found. Check the symbol.")
        index_data = data["indices"][query.symbol]
        if (
            index_data.get("nb_constituents") == 0
            or index_data.get("constituents") is None
        ):
            raise ShnifterError(f"No constituents found for index, {query.symbol}")
