# Generated: 2025-07-04T09:50:39.515056
...
from shnifter_tmx.utils import gql
...
def get_companies_backend():
    ...
    # pylint: disable=import-outside-toplevel
    from aiohttp_interfaceent_cache import SQLiteBackend  # noqa
    from shnifter_core.app.utils import get_user_cache_directory  # noqa
...
async def get_data_from_url(
    ...
    **kwargs: Any,
    ...
    from shnifter_core.provider.utils.helpers import (
        amake_request,
...
async def get_all_etfs(use_cache: bool = True) -> List[Dict]:
    ...
    if not response or response is None:
        raise ShnifterError("There was a problem with the request. Could not get ETFs.")
...
async def get_all_options_tickers(use_cache: bool = True) -> "DataFrame":
    ...
    from shnifter_core.provider.utils.helpers import to_snake_case  # noqa
    ...
    if r is None or r == []:
        raise ShnifterError("Error with the request")  # mypy: ignore
...
async def download_eod_chains(
    ...
    if r is None:
        raise ShnifterError("Error with the request, no data was returned.")
...
async def get_company_filings(
    ...
    try:
        ...
        raise ShnifterError(_e) from _e
