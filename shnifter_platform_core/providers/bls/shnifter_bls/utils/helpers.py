# Generated: 2025-07-04T09:50:39.933783
...
async def download_category_series_ids(category) -> Tuple[List, Dict]:
    ...
    from shnifter_core.provider.utils.errors import EmptyDataError
    from shnifter_bls.utils.constants import SURVEY_CATEGORY_MAP, SURVEY_NAMES
...
def open_asset(asset: str) -> Union["DataFrame", Dict]:
    ...
    if ".xz" not in asset and "series" in asset:
        ...
        raise ShnifterError(f"Asset '{asset}' not supported. Expected .json or .xz file.")

    assets_path = Path(str(files("shnifter_bls").joinpath("assets")))

    if not os.path.exists(assets_path.joinpath(asset)):
        raise ShnifterError(f"Asset '{asset}' not found.")
...
async def update_static_asset(category: str) -> None:
    ...
    from pathlib import Path
    from shnifter_core.app.model.abstract.error import ShnifterError
    from shnifter_bls.utils.constants import SURVEY_CATEGORY_NAMES
    from numpy import nan
    from pandas import DataFrame
    ...
    try:
        ...
        raise ShnifterError(f"Failed to download {category} -> {e}") from e
