# Generated: 2025-07-04T09:50:39.688716
...
def load_irfcl_symbols() -> dict:
    ...
    from shnifter_imf.utils.constants import load_symbols
...
def load_country_map() -> dict:
    ...
    try:
        ...
        raise ShnifterError(f"Failed to load IMF IRFCL country map: {e}") from e
...
def validate_countries(countries) -> str:
    ...
    if isinstance(countries, str):
        ...
        raise ShnifterError("Invalid countries list.")
    ...
    if not new_countries:
        raise ShnifterError("No valid countries found in the supplied list.")
...
def validate_symbols(symbols) -> str:
    ...
    from warnings import warn  # noqa
    from shnifter_core.app.model.abstract.error import ShnifterError
    from shnifter_imf.utils.constants import IRFCL_PRESET
    ...
    if isinstance(symbols, str):
        ...
        raise ShnifterError("Invalid symbols list.")
...
async def _get_irfcl_data(**kwargs) -> list[dict]:
    ...
    # pylint: disable=import-outside-toplevel
    from shnifter_core.app.model.abstract.error import ShnifterError
    from shnifter_imf.utils import constants
    from shnifter_imf.utils.helpers import get_data
    ...
    if not indicators and not countries:
        raise ShnifterError("Country is required when returning the complete dataset.")
    ...
    if not data:
        raise ShnifterError(f"No data found for '{indicator}' in '{countries}'.")
