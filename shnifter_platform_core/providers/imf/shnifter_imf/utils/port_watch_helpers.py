# Generated: 2025-07-04T09:50:39.689823
...
from shnifter_imf.utils.constants import (
...
def get_port_ids_by_country(country_code: str) -> str:
    ...
    json_path = (
        importlib.resources.files("shnifter_imf") / "assets" / "imf_ports_by_country.json"
    )
...
def get_port_id_choices() -> list:
    ...
    json_path = (
        importlib.resources.files("shnifter_imf") / "assets" / "imf_portid_map.json"
    )
...
async def get_daily_chokepoint_data(
    ...
    from shnifter_core.provider.utils.helpers import get_async_requests_session
    ...
    async with await get_async_requests_session() as session:
        async with await session.get(url) as response:
            ...
            if response.status != 200:
                raise ShnifterError(f"Failed to fetch data: {response.status}")
...
async def get_all_daily_chokepoint_activity_data(
    ...
    async def _get_one_chokepoint_data(chokepoint_id):
        ...
        try:
            ...
            raise ShnifterError(f"Failed to fetch data for {chokepoint_id}: {e}") from e
    ...
    try:
        ...
        for result in gather_results:
            if isinstance(result, (ShnifterError, Exception)):
                raise result

        if not chokepoints_data:
            raise ShnifterError("All requests were returned empty.")
...
async def get_all_daily_port_activity_data() -> list:
    ...
    url = (
        "https://core.arcgis.com/api/v3/datasets/959214444157458aad969389b3ebe1a0_0/"
    ...
    try:
        async with await get_async_requests_session(
            ...
            if response.content is None:
                raise ShnifterError("No content returned from the request.")
        ...
        raise ShnifterError(f"Error fetching port activity data: {e} -> {e.args}") from e
