# Generated: 2025-07-04T09:50:39.685918
...
def load_country_map() -> dict:
    ...
    try:
        ...
        raise ShnifterError(f"Failed to load IMF DOT country map: {e}") from e
...
def load_country_to_code_map() -> dict:
    ...
    except ShnifterError:
        warn(f"Failed to load country to code map. -> {ShnifterError}")
        return {}
...
def validate_countries(countries: list[str]) -> str:
    ...
    try:
        ...
        raise ShnifterError(f"Failed to load country to code map: {e}") from e
    if not country_to_code_map:
        raise ShnifterError("Failed to load country to code map.")
