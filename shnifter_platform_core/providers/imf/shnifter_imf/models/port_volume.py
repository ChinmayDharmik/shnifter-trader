# Generated: 2025-07-04T09:50:39.682913
...
from shnifter_core.provider.standard_models.port_volume import (
    PortVolumeData,
...
from shnifter_imf.utils.port_watch_helpers import (
    get_port_id_choices,
...
class ImfPortVolumeQueryParams(PortVolumeQueryParams):
    ...
    Source: UN Global Engine; IMF PortWatch (portwatch.imf.org)
    ...
    port_code: Optional[str] = Field(
        ...
        + " To get a list of available ports, use `shnifter.economy.shipping.port_info()`.",
    )
    ...
    def validate_port_code(cls, v):
        ...
        if not isinstance(v, list) or not all(isinstance(item, str) for item in v):
            raise ShnifterError("port_code must be a string or a list of strings.")
...
class ImfPortVolumeFetcher(Fetcher[ImfPortVolumeQueryParams, list[ImfPortVolumeData]]):
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        import asyncio  # noqa
        from shnifter_imf.utils.port_watch_helpers import get_daily_port_activity_data
        ...
        if not port_codes:
            raise ShnifterError("Expected values as valid portIDs, got None instead.")
