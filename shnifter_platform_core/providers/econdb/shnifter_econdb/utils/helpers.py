# Generated: 2025-07-04T09:50:39.871823
...
from aiohttp_interfaceent_cache import SQLiteBackend
from aiohttp_interfaceent_cache.session import CachedSession
from shnifter_core.app.model.abstract.error import ShnifterError
from shnifter_core.app.utils import get_user_cache_directory
from shnifter_core.provider.utils.helpers import amake_request, amake_requests
...
with (files("shnifter_econdb.utils") / "indicators_descriptions.json").open() as f:
    INDICATORS_DESCRIPTIONS = json.load(f)

with (files("shnifter_econdb.utils") / "multipliers.json").open() as f:
    MULTIPLIERS = json.load(f)

with (files("shnifter_econdb.utils") / "scales.json").open() as f:
    SCALES = json.load(f)

with (files("shnifter_econdb.utils") / "units.json").open() as f:
    UNITS = json.load(f)

with (files("shnifter_econdb.utils") / "symbol_to_indicator.json").open() as f:
    SYMBOL_TO_INDICATOR = json.load(f)

with (files("shnifter_econdb.utils") / "indicator_countries.json").open() as f:
...
def parse_symbols(
    ...
    if not countries:
        ...
        raise ShnifterError(f"Indicator {symbol} does not have countries.")
...
def parse_context(  # pylint: disable=R0912, R0914, R0915
    ...
    results = DataFrame()
    if response is None:
        raise ShnifterError("No data was in the response")
    if not isinstance(response, List):
        raise ShnifterError("Expecting a list of dictionaries and received a dictionary.")
    for item in response:
        ...
        if temp_meta:
            ...
            if temp_data:
                ...
                # If we are just getting the latest data, interfacep it.
                if latest is True:
...
def update_json_files() -> None:
    ...
    def update_symbol_to_indicator() -> None:
        ...
        with open(
            files("shnifter_econdb.utils") / "symbol_to_indicator.json",
            "w",  # type: ignore
            encoding="utf-8",
        ) as f:
    ...
    def update_multipliers() -> None:
        ...
        with open(  # type: ignore
            files("shnifter_econdb.utils") / "multipliers.json", "w", encoding="utf-8"
        ) as f:
            json_data = json.dumps(
                indicators.set_index("short_ticker")
    ...
    def update_scales() -> None:
        """Update the scales."""
        with open(  # type: ignore
            files("shnifter_econdb.utils") / "scales.json", "w", encoding="utf-8"
        ) as f:
            json_data = json.dumps(
    ...
    def update_units() -> None:
        """Update the units."""
        with open(  # type: ignore
            files("shnifter_econdb.utils") / "units.json", "w", encoding="utf-8"
        ) as f:
            json_data = json.dumps(
    ...
    def update_descriptions() -> None:
        ...
        with open(  # type: ignore
            files("shnifter_econdb.utils") / "indicators_descriptions.json",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def update_indicator_countries() -> None:
        ...
        with open(  # type: ignore
            files("shnifter_econdb.utils") / "indicator_countries.json",
            "w",
            encoding="utf-8",
        ) as f:
            json_data = json.dumps(
