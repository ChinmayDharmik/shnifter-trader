# Generated: 2025-07-04T09:50:39.543598
...
from shnifter_core.provider.standard_models.company_filings import (
    CompanyFilingsData,
...
from shnifter_sec.utils.definitions import FORM_LIST, HEADERS
...
class SecCompanyFilingsQueryParams(CompanyFilingsQueryParams):
    ...
    def validate_form_type(cls, v):
        ...
        if isinstance(v, str):
            ...
            raise ShnifterError("Unexpected form_type value.")
        new_forms: list = []
        messages: list = []
        for form in forms:
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        if query.symbol and not query.cik:
            ...
            if not query.cik:
                raise ShnifterError(f"CIK not found for symbol {query.symbol}")
        if query.cik is None:
            raise ShnifterError("CIK or symbol must be provided.")
