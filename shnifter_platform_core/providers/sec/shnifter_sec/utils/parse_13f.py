# Generated: 2025-07-04T09:50:39.567437
...
async def get_13f_candidates(symbol: Optional[str] = None, cik: Optional[str] = None):
    ...
    if len(filings) == 0:
        raise ShnifterError(f"No 13F-HR filings found for {symbol if symbol else cik}.")
...
async def complete_submission_callback(response, _):
    ...
    raise ShnifterError(f"Request failed with status code {response.status}")
...
async def get_complete_submission(url: str):
    ...
    from shnifter_sec.utils.definitions import HEADERS
