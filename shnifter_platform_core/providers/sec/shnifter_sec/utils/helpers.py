# Generated: 2025-07-04T09:50:39.566418
...
from shnifter_core.provider.utils.helpers import amake_request, make_request
from shnifter_sec.utils.definitions import HEADERS, SEC_HEADERS
...
async def get_series_id(
    ...
    if not symbol and not cik:
        raise ShnifterError("Either symbol or cik must be provided.")
...
async def get_nport_candidates(symbol: str, use_cache: bool = True) -> List[Dict]:
    ...
    try:
        ...
        raise ShnifterError("Fund not found for, the symbol: " + symbol) from e
    if series_id == "" or series_id is None:
        raise ShnifterError("Fund not found for, the symbol: " + symbol)
